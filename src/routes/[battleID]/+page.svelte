<script>
    ...
    async function createGameScene() {
        ...
        return class CardGameScene extends Phaser.Scene {
            ...
            createCharacter(x, y, team, position) {
                ...
                character.setScale(0.88)

                const characterImage = this.add.image(0, 0, `charater${team}${position}`);

                characterImage.setPosition(0, -character.displayWidth * 0.1);
                characterImage.setOrigin(0.5);
                
                //Set up scale cover crop for character image
                const scale = Math.max(
                    (character.displayWidth ) / characterImage.width,
                    (character.displayHeight ) / characterImage.height
                )*0.6;
                characterImage.setScale(scale);

                characterImage.setCrop(
                    (characterImage.width - character.displayWidth/scale) / 2 +20,
                    (characterImage.height - character.displayWidth/scale) / 2,
                    character.displayWidth/scale -40,
                    character.displayWidth/scale 
                );
                ...
                faintedOverlay.setPosition(characterImage.x, characterImage.y);


                const barWidth = character.displayWidth -36;
                const barHeight = 20;
                ...
                hpBarBg.setOrigin(0,-2.8);
                hpBarFg.setOrigin(0,-2.8);
                ...
                const fontSizeHp = 13;
                const fontSizeAtk = 11;
                const fontSizeRes = 11;
                
                const hpText = this.add.text(offsetHp, offsetY1, '', { fontSize: `${fontSizeHp}px`, fill: '#674F1E', fontStyle:"bold"});
                const atkText = this.add.text(offsetAtk, offsetY2, '', { fontSize: `${fontSizeAtk}px`, fill: '#0BF1DE', fontStyle:"bold"});
                const resText = this.add.text(offsetRes, offsetY3, '', { fontSize: `${fontSizeRes}px`, fill: '#0BF1DE', fontStyle:"bold"});
            ...
            createPassive(effectName, container) {
                const passiveSprite = this.add.sprite(0, -100, effectName);
            ...
            highlightCharacter(character,attribute) {
                ...
                const moveAmount = container.y < 360 ? 50 : -50;
            ...
            createProjectile(fromX, fromY, toX, toY, attributeName) {
                return new Promise((resolve) => {
                    ...
                    const targetZone = this.add.circle(toX, toY, 20);
                        ...
                        return new Promise(async (resolve, reject) => {
                            ...
                            await new Promise(resolve => {
                                this.showFloatingText(
                                    targetChar.container.x + 50,
                                    targetChar.container.y +50,
                        ...
                        try {
                            ...
                            if (container) {
                                ...
                                if (characterImage) {
                                    ...
                                    characterImage.setCrop(
                                        (characterImage.width - character.displayWidth/scale) / 2 ,
                                        (characterImage.height - character.displayWidth/scale) / 2,
                                        character.displayWidth/scale,
                                        character.displayWidth/scale
                                    );
            ...
            preloadNFTManually(key, url) {
                return new Promise((resolve) => {
                    ...
                    const timeout = setTimeout(() => {
                        ...
                        fallbackImg.src = '/game/elements/noImage.svg';
                    }, 5000);
    ...
    onMount(async () => {
        ...
        const isCurrentBattle = !battleID || battleID === 'battle';
        console.log()

        // If no battleID, require wallet connection
        if (isCurrentBattle) {
            ...
            // If battleID provided, wallet is optional
        ...
        const response = await fetch(config.rpcUrl, {
            method: "POST",
            body: JSON.stringify(!isCurrentBattle ? {
                ...
                <div class="absolute flex justify-center items-center z-20 h-full w-full bg-black bg-opacity-70" in:fade={{ duration: 300 }}>
                    <div class="flex flex-col justify-center items-center w-[40vw] h-[15vw] bg-darkStone self-center  border-[0.2vw] border-lightStone rounded-md">
                       {#if userAddress && (userAddress === challengerAddress || userAddress === defenderAddress) && userAddress === winnerAddress && winner!==-1}
                        ...
                        {:else if userAddress && (userAddress === challengerAddress || userAddress === defenderAddress) && userAddress !== winnerAddress && winner!==-1 }                            <img alt="You Lose" class="h-[5vw]" src="/game/elements/youLose.svg"/>
                            <div class="flex items-center gap-[0.5vw]">
                        ...
                        {:else if userAddress && (userAddress === challengerAddress || userAddress === defenderAddress) && winner ===-1 }                            <span></span>
                            <div class="flex items-center gap-[0.5vw]">
                ...
                <div class="flex flex-col h-full w-full relative px-[0.5vw]">
                    ...
                    <div class="grid grid-cols-2 gap-[1vw] mb-[0.3vw]">
                        <div class="flex gap-[1vw] items-center justify-center p-[0.2vw]">
                            ...
                            <img class="h-[1vw] ml-[0.3vw]" alt="ATK" src="/game/elements/noImage.svg"/>
                        </div>
                        <div class="flex gap-[1vw] items-center justify-center p-[0.2vw]">
